<section>
    ...
    <form method="post" action="{{ route('perfil.store') }}" class="mt-6 space-y-6" enctype="multipart/form-data">
        ...
        @else
            ...
            <div>
                <x-input-label for="email" :value="__('Ocupação')" />
                <x-text-input id="ocupacao" name="ocupacao" type="text" class="mt-1 block w-full" required autocomplete="ocupacao" />
                <x-input-error class="mt-2" :messages="$errors->get('ocupacao')" />
            </div>

            <div>
                <x-input-label for="email" :value="__('Descrição Pessoal')" />
                <x-text-input id="email" name="descricao_pessoal" type="text" class="mt-1 block w-full" required autocomplete="descricao_pessoal" />
                <x-input-error class="mt-2" :messages="$errors->get('descricao_pessoal')" />
            </div>

            <div>
                <x-input-label for="email" :value="__('Telefone')" />
                <x-text-input id="telefone" name="telefone" type="text" class="mt-1 block w-full" required autocomplete="username" placeholder="(00)00000-0000"/>
                <x-input-error class="mt-2" :messages="$errors->get('telefone')" />
            </div>

            <div>
                <x-input-label for="email" :value="__('Endereço')" />
                <x-text-input id="endereco" name="endereco" type="text" class="mt-1 block w-full" required autocomplete="username" />
                <x-input-error class="mt-2" :messages="$errors->get('endereco')" />
            </div>

            <div>
                <x-input-label for="email" :value="__('Cidade')" />
                <x-text-input id="cidade" name="cidade" type="text" class="mt-1 block w-full" required autocomplete="username" />
                <x-input-error class="mt-2" :messages="$errors->get('cidade')" />
            </div>

            <div>
                <x-input-label for="email" :value="__('Estado')" />
                <x-text-input id="estado" name="estado" type="text" class="mt-1 block w-full" required autocomplete="username" />
                <x-input-error class="mt-2" :messages="$errors->get('estado')" />
            </div>

            <div>
                <x-input-label for="email" :value="__('CEP')" />
                <x-text-input id="cep" name="cep" type="text" class="mt-1 block w-full"  required autocomplete="username" placeholder="00000-000"/>
                <x-input-error class="mt-2" :messages="$errors->get('cep')" />
            </div>
